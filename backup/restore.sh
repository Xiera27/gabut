...
NC='\e[0m'
###########- END COLOR CODE -##########
cekdata=$(curl -sS https://raw.githubusercontent.com/Xiera27/backup/main/$NameUser/$NameUser.zip | grep 404 | awk '{print $1}' | cut -d: -f1)
clear
cd
#link izin ip vps
...
wget -q -O /root/backup/backup.zip "https://raw.githubusercontent.com/Xiera27/backup/main/$nama/$nama.zip" &> /dev/null
...
echo -e "$COLOR1│${NC}  [ ${green}INFO${NC} ] • Done... Successfully - Script By Xiera"
sleep 1
rm -f /root/backup/backup.zip &> /dev/null
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
echo ""
